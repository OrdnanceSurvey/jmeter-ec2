						...
						if [ -z "$count" ] ; then count=1 ; fi
							...
							function check_prereqs() {
								...
								if [ -f "$project_home/jmx/$project.jmx" ] ; then
									...
									if  ! type aws ec2 describe-instances &>/dev/null  ; then
										...
										if [ -z "$instance_count" ] ; then instance_count=1; fi
											...
											if [ -n "$SUBNET_ID" ] ; then vpcsettings="-s $SUBNET_ID --associate-public-ip-address \"true\""; fi
												...
												if [ -z "$price" ] ; then
													...
													--image-id $AMI_ID \
													| grep InstanceId | sed s,"\"InstanceId\": \"",, | sed s,"\"\,",, | sed 's,^[ \t]*,,' | sed s," ",,`)
												else
													...
													--spot-price $price \
													--launch-specification "{\"InstanceType\":\"${INSTANCE_TYPE}\",\"ImageId\":\"${AMI_ID}\",\"KeyName\":\"${AMAZON_KEYPAIR_NAME}\",\"NetworkInterfaces\":[{\"DeviceIndex\":0,\"SubnetId\":\"${INSTANCE_SUBNET_ID}\",\"AssociatePublicIpAddress\":true,\"Groups\":[\"$INSTANCE_SECURITYGROUP\"]}]}" \
													--instance-count $instance_count \
													| grep SpotInstanceRequestId | sed s,"\"SpotInstanceRequestId\": \"",, | sed s,"\"\,",, | sed 's,^[ \t]*,,' | sed s," ",,`)
													...
													do
														spot_request_statuses=(`aws ec2 describe-spot-instance-requests --spot-instance-request-ids ${spot_instance_request_id[@]} | grep Code | sed s,"\"Code\": \"",, | sed s,"\"\,",, | sed 's,^[ \t]*,,' | sed s," ",,`)
														...
														if [[ "$spot_request_error_count" = "${#spot_instance_request_id[@]}" ]]; then
															...
															aws ec2 cancel-spot-instance-requests --instance-id $(printf " %s" "${spot_instance_request_id[@]}")
															exit
														fi
													...
													spot_id_filter="Name=spot-instance-request-id,Values="
													for x in "${spot_instance_request_id[@]}" ; do
														spot_id_filter="${spot_id_filter}${x},"
													done
													...
													while true; do
														instance_describe=`aws ec2 describe-instances --filters $spot_id_filter`
													...
													attempted_instanceids=(`aws ec2 describe-instances --filter $spot_id_filter | grep InstanceId | sed s,"\"InstanceId\": \"",, | sed s,"\"\,",, | sed 's,^[ \t]*,,' | sed s," ",,`)
												fi
												...
												do
													echo -n .
													status_check_count=$(( $status_check_count + 1))
													count_passed=$(aws ec2 describe-instance-status --instance-id ${attempted_instanceids[@]} | grep Status | sed s,"\"Status\": \"",, | sed s,"\"\,",, | sed 's,^[ \t]*,,' | sed s," ",, | grep -c ok)
													sleep 3
												done
												...
												if [ $status_check_count -lt $status_check_limit ] ; then # all hosts started ok because count_passed==instance_count
													...
													# set hosts array
													hosts=(`aws ec2 describe-instances --instance-ids ${attempted_instanceids[@]} | grep SpotInstanceRequestId | sed s,"\"SpotInstanceRequestId\": \"",, | sed s,"\"\,",, | sed 's,^[ \t]*,,' | sed s," ",,`)
													...
													hosts=(`aws ec2 describe-instances --instance-ids ${healthy_instanceids[@]} | awk '/INSTANCE/ {print $4}'`)
												...
												echo "assigning tags..."
												(aws ec2 create-tags --resources ${attempted_instanceids[@]} --tags Key=Product,Value=$project Key=Service,Value=prod KeyDescription,Value=PerformanceTest Key=Owner,Value=$EMAIL Key=ContactEmail,Value=$EMAIL Key=Name,Value="jmeter-ec2-$project")
												wait
												echo "complete"
												echo
												...
												if [ ! -z "$ELASTIC_IPS" ] ; then # Not Null - same as -n
													...
													for x in "${!instanceids[@]}" ; do
														(aws ec2 associate-address --instance-id ${instanceids[x]} --public-ip ${elasticips[x]})
